from app import app
import game.routes as routes


def setup_function():
    """Reset the game before each test."""
    routes.board = [' '] * 9
    routes.current_player = 'X'
    routes.move_history = []
...
def test_play_adds_move_to_stack():
    ...
    assert routes.board[0] == 'X'
    assert routes.move_history == [(0, 'X')]
    assert routes.current_player == 'O'
...
def test_reset_clears_move_history_stack():
    ...
    assert routes.board == [' '] * 9
    assert routes.move_history == []
    assert routes.current_player == 'X'